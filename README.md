...
├── 可行性分析.md         # Android 电话 API 能力边界、四种架构、竞品调研
├── 部署方案.md           # 双机声学耦合方案、电脑方案概要
└── 电脑方案详解.md       # 本方案的硬件/软件/运维细节
```
...
**目标**：让低成本笔记本 + 一部安卓手机组成"AI 秘书"，不在场时自动接听电话、播报问候语、采集留言并生成摘要。

**现状**（2026-09-22）：单机全链路已在真机（一加 Ace 5 至尊版 + MacBook Air + Blue Yeti）上验证——来电检测、10 秒自动接听、问候语播放、留言录音均正常；录音信噪比经带通滤波与摆位调优后可用；转写与摘要依赖（whisper/ollama）尚未在验证机上安装，代码已就绪。

## 文档导览

| 文档 | 内容 |
|---|---|
| [可行性分析.md](可行性分析.md) | 为什么这么做：Android 电话 API 的硬限制、四条实现架构对比、市面竞品（Pixel Call Screen / Bixby / 小爱通话 / Truecaller Assistant）的实现方式 |
| [部署方案.md](部署方案.md) | 双机声学耦合变体、外置麦克风（Yeti）接入、与电脑方案的对比 |
| [电脑方案详解.md](电脑方案详解.md) | 本文方案的完整工程细节：硬件清单、软件栈、摆位图、日常运维 |
| [code/README.md](code/README.md) | 代码文档：安装、配置项、命令、已知限制、故障速查 |
